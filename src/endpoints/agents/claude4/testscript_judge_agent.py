...
import logging
import json
from endpoints.prompts.claude4.prompts import get_testscript_judge_system_message, get_testscript_judge_prompt

logger = logging.getLogger(__name__)


class TestScriptJudgeAgentClaude4:
    """
    LLM-based judge agent that reviews and evaluates test scripts using Claude Sonnet.
    Evaluates test scripts based on accuracy, consistency, factualness, completeness, and other quality metrics.
    """

    def __init__(self, api_key: str = None):
        """
        Initialize the TestScriptJudgeAgent.
        ...
        # Initialize LLM client for Claude Sonnet
    ...
    def review_test_scripts(
        ...
        Args:
            test_case: Test case dictionary for context
            test_script: Test script string to review
            api_spec: Optional API specification for context
            
        Returns:
            Dictionary containing review results with scores and feedback for the test script
        """
        try:
            # Create review prompt using new prompt functions
            prompt = get_testscript_judge_prompt(test_case, test_script)
                        
            # Call Claude Sonnet for review
            completion = self.llm_client.chat.completions.create(
                ...
                temperature=temperature  # Lower temperature for more consistent, objective reviews
            )
            ...
            try:
                ...
                return {
                    ...
                    "test_case": test_case.code if 'code' in test_case else 'N/A'
                }
